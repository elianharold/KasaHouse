...
export const chatService = {
  ...
  async startThread(listingId: string): Promise<ChatThreadDetail> {
    const { data } = await api.post<ChatThreadDetail>(API_ROUTES.chat.start, {
      listingId,
    });
    return data;
  },
  async getThread(id: string): Promise<ChatThreadDetail> {
  ...
  async sendMessage(id: string, content: string): Promise<ChatMessage> {
    const { data } = await api.post<ChatMessage>(API_ROUTES.chat.messages(id), {
      content,
    });
    return data;
  },
